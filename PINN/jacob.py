...
def pde(x, y):  #here x is the input (x,y,z coordinates) of the model and y the output (pressure)
    ...
    Ly = x[:, 4:5] * L_range[1] + L_min[1]
    Lz = x[:, 5:6] * L_range[2] + L_min[2]
    ...
    y0_z = dde.grad.jacobian(y, x, i=0, j=2)
    ...
    y0_yy = dde.grad.jacobian(y0_y, x, j=1) / y_dim_sq
    y0_zz = dde.grad.jacobian(y0_z, x, j=2) / z_dim_sq
    ...
    x_dist = pos_x - pos_xs # precompute for more readable squaring
    y_dist = pos_y - pos_ys
    z_dist = pos_z - pos_zs
...
data = ValidationPDE(
    ...
    num_boundary=0, # we dont have any training points on the boundary
    anchors=X_train,
    validation_x=X_val,
