...
def create_RTF_grid(directory, target_freq, heights, freq_axis_key='freqs', rtf_key='TransferFunction'):
    grid = np.zeros((32, 32, len(heights)), dtype=complex)
    approximated_freq = None

    if not os.path.isdir(directory):
        raise ValueError(f"Directory {directory} does not exist")

    for height_idx, height_val in enumerate(heights):
        dir_h = os.path.join(directory, f"h_{height_val}")
        if not os.path.isdir(dir_h):
            raise ValueError(f"Directory {dir_h} does not exist")

        for entry in os.scandir(dir_h):
            ...
            freq_axis = mat[freq_axis_key].flatten()
            rtf = mat[rtf_key].flatten()

            idx = np.argmin(np.abs(freq_axis - target_freq))
            grid[idxX-1, idxY-1, height_idx] = rtf[idx]

            if approximated_freq is None:
                approximated_freq = freq_axis[idx]
