""" 
Uses all sources and all heights, modeled as a 3D problem
It takes normalized x,y,z coordinates and room dimensions as input, output is the predicted real and imaginary part of the FFT at the specified target frequency.
...
data = dde.data.PDE(
    ...
    num_domain=0, #evaluates physics strictly on the coordinates specified in anchors
    num_boundary=0, 
    anchors=X
)
...
model.compile("adam", lr=1e-3, loss="MSE", loss_weights=[1, 1, 60, 60]) 
