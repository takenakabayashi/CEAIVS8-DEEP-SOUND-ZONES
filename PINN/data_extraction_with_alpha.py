...
def extract_data_simulated(df, target_freq, file_path=SIMULATED_DATA_FILE, max_points_per_room=None): #add absorption coeff as input?
    ...
    with h5py.File(file_path, "r") as f:
        for _, row in df.iterrows():
            ...
            '''
            # load all frequencies and then pick the needed one
            ...
            '''
            # only load the needed frequency (memory is the same anyways ???)
            receiver_pos = grp["receiver_pos"][:]  # (R, 3)
            source_pos = grp["source_pos"][:]      # (S, 3)
            room_dim = grp["room_dim"][0]
            freqs = grp["freqs"][0]
            alpha = float(grp["alpha"][0, 0])

            # Load only the needed frequency
            freq_idx = np.argmin(np.abs(freqs - target_freq))
            real = grp["RTF_real"][freq_idx, :, :]   # (S, R)
            imag = grp["RTF_imag"][freq_idx, :, :]   # (S, R)
