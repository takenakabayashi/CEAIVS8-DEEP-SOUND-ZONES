...
parfor i = 1:N_rooms
    ...
    % receivers
    rx_vec = build_axis(Lx, src_margin, grid_step);
    ry_vec = build_axis(Ly, src_margin, grid_step);

    ceiling_margin = 0.3;
    height_vec_full = 0.8 : grid_step : 1.8;
    height_vec = height_vec_full(height_vec_full <= Lz - ceiling_margin); 
...
%% =========================================================
function v = build_axis(L, margin, step)
% Returns receiver positions on axis of length L guaranteed 
% to be at least 'margin' from both walls
usable = L - 2 * margin;
if usable < 0
    v = [];
    return;
...
n = floor(usable / step) + 1;
span = (n - 1) * step;
start = margin + (usable - span) / 2;
v = start + (0:n-1) * step;
end